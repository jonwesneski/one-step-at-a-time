...
if (typeof window !== 'undefined' && typeof customElements !== 'undefined') {
  class StaffTrebleElement extends StaffElementBase {
    static #trebleClefSvg = `
      <svg class="clef" height="60px" width="30px" version="1.1" xmlns="www.w3.org" xmlns:xlink="www.w3.org" viewBox="150 0 165.4 496.2" xml:space="preserve" stroke="currentColor" transform="translate(0, 24)">
    ...
    `;
    static #trebleYCoordinates: { [x in string]: number } = {
      // Above 1st line
      C6: 10,
      B5: 15,
      A5: 20,
    ...
    public getYCoordinate(note: string): number {
      ...
      // direct match
      if (StaffTrebleElement.#trebleYCoordinates[key] !== undefined) {
        return StaffTrebleElement.#trebleYCoordinates[key];
      }

      // try with a suffix like '4' if the user provided octave info loosely
      for (const n of [4, 5, 6]) {
        if (
          StaffTrebleElement.#trebleYCoordinates[`${key}${n}`] !== undefined
        ) {
          return StaffTrebleElement.#trebleYCoordinates[`${key}${n}`];
        }
      }
