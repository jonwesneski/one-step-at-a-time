...
- `measure.ts`'s `#renderGroupConnectors()` turns each resolved pair into a positioned `createBraceSvg()`/`createBracketSvg()` glyph (`utils/svgCreator/brace.ts`) in a `.group-connectors` overlay, re-run alongside the existing resize-driven `#updateConnectorVisibility()` pass (same reactivity model as the plain barline — recomputed on resize, not on live attribute changes).
