...
- **Octave signs (8va/15ma/loco) and note clusters (Cowell-style black-key/white-key/chromatic notation) are not implemented.** Both appear frequently in keyboard writing but are general-purpose notations, not keyboard-specific, and each is a substantial standalone feature — tracked as their own unstarted rows in `TODO.md` (§5 "Octave Lines", §13 "Special / Extended Techniques" respectively), deliberately out of scope for the keyboard/cross-staff work above.
...
| `Instruments/Keyboard`                                                       | `src/keyboard.stories.ts` — cross-staff slurs, glissando, shared/per-voice dynamics, hand-distribution labels (see Keyboard / Cross-Staff Notation above); a `Planned` story notes pedal marks, octave signs, and note clusters as not yet implemented                        |
